...
## スーパークラス
### スーパークラス(は、親クラス)
コンストラクタの中では一行目にスーパークラスのコンストラクタか  
自分自身のコンストラクタを呼び出さなければならない  
```super()```
```this()```
書いてないと暗黙で```super()```が呼び出される  
何も継承してないクラスの場合は暗黙でObjectクラスが継承されている  
...
①▲▲クラスの宣言にabstractを付けて抽象クラスにする。  
②▲▲クラス内部の「未完成部分」をすべてなくす  

②をしてすべてのメソッドの動作が実装されて詳細未定な部分が残っていない場合、  
newして使える通常のクラスになった  
...
### インタフェースの効果
<span style="color: red; ">①同じインタフェースをimplementsする複数の子クラスたちに、  
...
多重継承が許されている　　
テスト穴埋めで  
interfaceのフェースをsにするひとがいるけど気を付けて　　
...
```ArrayList```は```get```メソッドを使って値を取得する  
